...
@section('title', 'Edit Akun Auditor - Moto Audit System')
...
<div class="space-y-6 pb-12 max-w-3xl">
    ...
    <div class="border-b border-gray-200 pb-5">
        ...
        <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Akun Pengguna</h1>
        <p class="text-sm text-gray-500 mt-1">Perbarui data akun pengguna {{ $user->name }} ({{ $user->nik }})</p>
    </div>

    {{-- Form Card --}}
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('admin.auditor.update', $user->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-1">
                    Password Baru <span class="text-xs font-normal text-gray-500">(Kosongkan jika tidak ingin mengubah password)</span>
                </label>
                <input type="password" 
                       name="password" 
                       id="password" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="role" 
                        ...
                        <option value="auditor" {{ old('role', $user->role) === 'auditor' ? 'selected' : '' }}>Auditor (Standard User)</option>
                        <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Admin QA (Super Admin)</option>
                    </select>
                    @error('role')
                        <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="tipe_auditor" 
                        ...
                        <option value="auditor" {{ old('tipe_auditor', $user->tipe_auditor) === 'auditor' ? 'selected' : '' }}>Auditor</option>
                        <option value="instruktur" {{ old('tipe_auditor', $user->tipe_auditor) === 'instruktur' ? 'selected' : '' }}>Instruktur</option>
                    </select>
                    @error('tipe_auditor')
                        <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                    @enderror
