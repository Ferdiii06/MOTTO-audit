...
@section('title', 'Tambah Jenis Audit - Moto Audit System')
...
<div class="space-y-6 pb-12 max-w-3xl">
    ...
    <div class="border-b border-gray-200 pb-5">
        ...
        <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Tambah Jenis Audit Baru</h1>
        <p class="text-sm text-gray-500 mt-1">Buat jenis audit baru untuk pengelompokan area audit</p>
    </div>

    {{-- Form Card --}}
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('admin.jenis-audit.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       id="nama" 
                       value="{{ old('nama') }}"
                       placeholder="Contoh: 5S Standard"
                       required
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       id="slug" 
                       value="{{ old('slug') }}"
                       placeholder="Contoh: 5s_standard"
                       required
            ...
            <div>
                ...
                <textarea name="deskripsi" 
                          id="deskripsi" 
                          rows="4" 
                          placeholder="Penjelasan singkat mengenai jenis audit ini..."
                          class="w-full px-4 py-2.5 rounded-xl border @error('deskripsi') border-rose-400 bg-rose-50/30 @else border-gray-300 @enderror text-sm focus:ring-2 focus:ring-yazaki-red focus:border-yazaki-red outline-none transition-all">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
