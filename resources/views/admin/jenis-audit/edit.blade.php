...
<div class="space-y-6 pb-12">
    ...
    <section class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <button type="button" onclick="document.getElementById('add-area-form').classList.toggle('hidden')" class="px-4 py-2 rounded-xl bg-yazaki-red text-white text-sm font-semibold">+ Tambah Area</button>
        </div>
                ...
                <details class="bg-white border border-gray-200 rounded-xl shadow-sm" id="area-{{ $area->id }}">
                    ...
                    <div class="border-t border-gray-100 p-5 space-y-4">
                        ...
                        <form id="add-process-{{ $area->id }}" action="{{ route('admin.process.store', $area->id) }}" method="POST" class="hidden bg-gray-50 rounded-lg p-4 space-y-2">@csrf
                            <input name="name" required placeholder="Nama proses" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm"><textarea name="description" placeholder="Deskripsi" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm"></textarea><textarea name="checkpoint" placeholder="Checkpoint" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm"></textarea><textarea name="kriteria_judgement" placeholder="Kriteria judgement" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm"></textarea><button class="px-3 py-2 rounded-lg bg-yazaki-red text-white text-xs font-semibold">Simpan Proses</button>
                        </form>
                        <div class="divide-y divide-gray-100">
                            @forelse($area->processes as $process)
                                <div class="py-3 flex justify-between gap-3"><div class="min-w-0"><p class="font-semibold text-sm text-gray-900">{{ $process->name }}</p><p class="text-xs text-gray-500 truncate">{{ $process->checkpoint ?: 'Tidak ada checkpoint' }}</p></div><div class="flex gap-2 shrink-0"><button type="button" onclick="document.getElementById('edit-process-{{ $process->id }}').classList.toggle('hidden')" class="px-2 py-1 rounded bg-gray-100 text-xs">Edit</button><form action="{{ route('admin.process.destroy', $process->id) }}" method="POST" onsubmit="return confirm('Hapus proses {{ addslashes($process->name) }}? {{ \App\Models\AuditSchedule::where('audit_process_id', $process->id)->count() }} schedule dapat ikut terhapus.')">@csrf @method('DELETE')<button class="px-2 py-1 rounded bg-rose-50 text-rose-700 text-xs">Hapus</button></form></div></div>
                                <form id="edit-process-{{ $process->id }}" action="{{ route('admin.process.update', $process->id) }}" method="POST" class="hidden pb-3 space-y-2">@csrf @method('PUT')<input name="name" value="{{ $process->name }}" required class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm"><textarea name="description" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm">{{ $process->description }}</textarea><textarea name="checkpoint" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm">{{ $process->checkpoint }}</textarea><textarea name="kriteria_judgement" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm">{{ $process->kriteria_judgement }}</textarea><button class="px-3 py-2 rounded-lg bg-yazaki-red text-white text-xs font-semibold">Simpan Proses</button></form>
