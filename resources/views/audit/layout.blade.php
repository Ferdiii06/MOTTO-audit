...
<body class="bg-gray-50 text-gray-800 min-h-screen flex flex-col md:flex-row overflow-x-hidden md:overflow-hidden">
    ...
    <aside id="mobile-sidebar-menu" class="fixed inset-y-0 left-0 z-40 w-64 bg-yazaki-red flex flex-col justify-between transform -translate-x-full transition-transform duration-300 ease-in-out md:hidden shrink-0">
        <div class="flex flex-col h-full overflow-y-auto">
            ...
            <nav class="p-3 space-y-1 flex-1 overflow-y-auto">
                ...
                <div class="pl-3 space-y-1 mt-1">
                    ...
                    @foreach($subItems as $sub)
                        <a href="{{ url(ltrim($sub['route'], '/')) }}"
                            class="flex items-center space-x-3 px-3 py-2 rounded-lg text-sm font-semibold transition-all {{ request()->is($sub['path']) ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ url('audit/riwayat') }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all {{ request()->is('audit/riwayat*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ url('audit/pedoman') }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all {{ request()->is('audit/pedoman*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if(session('audit_user_role') === 'admin')
                    <a href="{{ route('admin.jenis-audit.index') }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all {{ request()->is('audit/admin/jenis-audit*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                    ...
                    <a href="{{ route('admin.auditor.index') }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all {{ request()->is('audit/admin/auditor*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                        <span>Kelola Auditor</span>
                    </a>
                    <a href="{{ route('admin.jadwal.index') }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all {{ request()->is('audit/admin/jadwal*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
    ...
    <aside class="hidden md:flex w-52 bg-yazaki-red flex-col justify-between shrink-0 h-screen sticky top-0">
        <div class="flex flex-col h-full overflow-y-auto">
            {{-- Desktop Header --}}
            <div class="px-5 py-5 border-b border-white/20 shrink-0">
                <h2 class="text-white font-extrabold text-base tracking-tight">Audit System</h2>
                <p class="text-white/60 text-xs mt-0.5">PT Jatim Autocomp</p>
            </div>
            ...
            <nav class="p-3 space-y-1 flex-1 overflow-y-auto">
                ...
                <div class="flex items-center justify-between rounded-lg transition-all {{ request()->is('audit/dashboard') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                    ...
                    <button type="button" id="home-toggle-btn-desktop" class="px-2.5 py-2.5 text-current opacity-70 hover:opacity-100 transition-opacity focus:outline-none" aria-label="Toggle Home Submenu">
                ...
                <div id="home-submenu-desktop" class="pl-3 space-y-1 mt-1 {{ $isHomeGroupActive ? '' : 'hidden' }}">
                    @foreach($subItems as $sub)
                        <a href="{{ url(ltrim($sub['route'], '/')) }}"
                            class="flex items-center space-x-3 px-3 py-2 rounded-lg text-sm font-semibold transition-all {{ request()->is($sub['path']) ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
