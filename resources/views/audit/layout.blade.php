    ...
    <aside class="w-52 bg-yazaki-red flex flex-col justify-between shrink-0">
        <div>
            ...
            <nav class="p-3 space-y-1">
                ...
                <a href="/audit/dashboard"
                    ...
                    {{ request()->is('audit/dashboard') ? 'bg-white text-yazaki-red' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                ...
                {{-- Sub-items: only visible when inside Home group --}}
                    ...
                    <div class="pl-4 space-y-1">
                        ...
                        @foreach($subItems as $sub)
                            <a href="{{ $sub['route'] }}"
                                ...
                                {{ request()->is($sub['path']) ? 'bg-white/30 text-white' : 'bg-white/10 text-white/80 hover:bg-white/20 hover:text-white' }}">
                ...
                <a href="/audit/riwayat"
                    class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all
                    {{ request()->is('audit/riwayat') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="/audit/pedoman"
                    class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all
                    {{ request()->is('audit/pedoman') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-3 border-t border-white/20">
            <form method="POST" action="/audit/logout">
                @csrf
                <button type="submit" class="w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-white/70 hover:bg-white/10 hover:text-white transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
