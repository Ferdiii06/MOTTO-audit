...
<body class="bg-gray-50 text-gray-800 h-dvh md:h-[100dvh] flex flex-col md:flex-row overflow-x-hidden overflow-y-hidden md:overflow-hidden">
    ...
    <aside class="hidden md:flex w-52 bg-yazaki-red flex-col justify-between shrink-0 h-[100dvh] sticky top-0 overflow-hidden">
        {{-- Desktop Header --}}
        <div class="px-4 py-3.5 border-b border-white/20 shrink-0">
            <h2 class="text-white font-extrabold text-base tracking-tight">Audit System</h2>
            <p class="text-white/60 text-xs mt-0.5">PT Jatim Autocomp</p>
        </div>
        ...
        <nav class="p-2 space-y-0.5 flex-1 overflow-y-auto min-h-0">
            @php
                $homeSubGroup = ['audit/5s-standard*', 'audit/change-point-management*', 'audit/license-system*'];
                $isHomeSubActive = request()->is($homeSubGroup);
            @endphp

            <div class="flex items-center justify-between rounded-lg transition-all {{ request()->is('audit/dashboard') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                <a href="{{ url('audit/dashboard') }}" class="flex-1 flex items-center space-x-2.5 px-2.5 py-1.5 text-xs font-semibold">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                    </svg>
                    <span>Home</span>
                </a>
                <button type="button" id="home-toggle-btn-desktop" class="px-2 py-1.5 text-current opacity-70 hover:opacity-100 transition-opacity focus:outline-none" aria-label="Toggle Home Submenu">
                    <svg id="home-chevron-icon-desktop" class="w-3.5 h-3.5 transition-transform duration-200 {{ $isHomeSubActive ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
            </div>

            <div id="home-submenu-desktop" class="pl-2.5 space-y-0.5 mt-0.5 {{ $isHomeSubActive ? '' : 'hidden' }}">
                @foreach($subItems as $sub)
                    <a href="{{ url(ltrim($sub['route'], '/')) }}"
                        class="flex items-center space-x-2.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all {{ request()->is($sub['path']) ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ url('audit/riwayat') }}" class="flex items-center space-x-2.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all {{ request()->is('audit/riwayat*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <span>Riwayat</span>
            ...
            <a href="{{ url('audit/pedoman') }}" class="flex items-center space-x-2.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all {{ request()->is('audit/pedoman*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                </svg>
                <span>Pedoman</span>
            ...
            @if(session('audit_user_role') === 'admin')
                <a href="{{ route('admin.jenis-audit.index') }}" class="flex items-center space-x-2.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all {{ request()->is('audit/admin/jenis-audit*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('admin.auditor.index') }}" class="flex items-center space-x-2.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all {{ request()->is('audit/admin/auditor*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    <span>Kelola Auditor</span>
                </a>
                <a href="{{ route('admin.jadwal.index') }}" class="flex items-center space-x-2.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all {{ request()->is('audit/admin/jadwal*') ? 'bg-white text-yazaki-red shadow-sm font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
        ...
        <div class="p-2.5 border-t border-white/20 shrink-0 bg-yazaki-red">
            <form method="POST" action="{{ url('audit/logout') }}">
                @csrf
                <button type="submit" class="w-full flex items-center space-x-2.5 px-2.5 py-2 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all cursor-pointer">
