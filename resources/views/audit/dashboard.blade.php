...
<div class="space-y-6 sm:space-y-8">
    {{-- Header --}}
    <div>
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Audit Dashboard</h1>
        <p class="text-xs sm:text-sm text-gray-500 mt-1">Ringkasan performa dan riwayat aktivitas audit di PT Jatim Autocomp</p>
    </div>

    {{-- Stats Grid (4 Cards) --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        {{-- Card 1: Total Audit Bulan Ini --}}
        <div class="bg-white rounded-xl p-4 sm:p-5 border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl sm:text-3xl font-extrabold text-amber-600 mt-2">{{ $stats['pending_audit'] }}</p>
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl sm:text-3xl font-extrabold text-blue-600 mt-2">{{ $stats['avg_score'] }}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <a href="{{ url('audit/riwayat') }}" class="text-xs font-semibold text-yazaki-red hover:underline flex items-center space-x-1 self-start sm:self-auto">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-gray-600 min-w-[500px]">
                <thead class="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
                    <tr>
                        <th scope="col" class="px-4 sm:px-6 py-3.5">Tanggal</th>
                        <th scope="col" class="px-4 sm:px-6 py-3.5">Area</th>
                        <th scope="col" class="px-4 sm:px-6 py-3.5">Auditor</th>
                        <th scope="col" class="px-4 sm:px-6 py-3.5">Kondisi</th>
                        <th scope="col" class="px-4 sm:px-6 py-3.5 text-right">Status</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 bg-white">
                    @foreach($recentAudits as $row)
                        <tr class="hover:bg-gray-50/80 transition-colors">
                            <td class="px-4 sm:px-6 py-3.5 sm:py-4 font-medium text-gray-900 whitespace-nowrap">{{ $row['tanggal'] }}</td>
                            <td class="px-4 sm:px-6 py-3.5 sm:py-4 font-semibold text-gray-800">{{ $row['area'] }}</td>
                            <td class="px-4 sm:px-6 py-3.5 sm:py-4 text-gray-600">{{ $row['auditor'] }}</td>
